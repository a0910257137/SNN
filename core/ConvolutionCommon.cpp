...
namespace SNN
{
    ...
    std::pair<int, int> ConvolutionCommon::GetPadding(const std::shared_ptr<Tensor> tensor)
    {
        ...
        const std::vector<uint8_t> &kernelShape = tensor->KernelShape();

        if (tensor->GetPaddingType() == kPaddingSame)
        {
            int kernelHeightSize = (static_cast<int>(kernelShape[1]) - 1) * tensor->dilation(0) + 1;
            int kernelWidthSize = (static_cast<int>(kernelShape[2]) - 1) * tensor->dilation(1) + 1;
            int padNeedPadHeight = (outputShape[1] - 1) * tensor->stride(0) + kernelHeightSize - inputShape[0];
            int padNeededWidth = (outputShape[2] - 1) * tensor->stride(1) + kernelWidthSize - inputShape[1];
