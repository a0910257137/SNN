...
namespace SNN
{
    ...
    std::pair<std::vector<float>, std::vector<std::vector<float>>> PostProcessor::MTFDProcessor(std::pair<std::vector<std::vector<float>>, std::vector<std::vector<std::vector<float>>>> &results,
                                                                                                int i,
                                                                                                float *resizedRatios,
                                                                                                float *cls_x,
                                                                                                float *bbox_x,
                                                                                                ...
                                                                                                const std::vector<int> &cls_xShape)
    {
        ...
        std::vector<std::vector<float>> outputCoord(1, std::vector<float>(6, 0));
        outputCoord[0] = outputCoords[selectedIdx];
        // std::cout << outputCoords.size() << std::endl;
        std::pair<std::vector<std::vector<float>>, std::vector<std::vector<float>>> ParamsKps = GetParamsKps(i, x, outputCoord, xShape);
        std::vector<float> &param = ParamsKps.first[0];
        std::vector<float> &tran = ParamsKps.second[0];
        ...
        for (l = 0; l < detSize; l++)
        {
            ...
            for (; k_out < end; k_out++)
            {
                ...
                for (j = 0; j < kernel_size; j++)
                {
                    ...
                    {
                        for (k_in = 0; k_in < input_channel; k_in = (k_in + 4))
                        {
    ...
    std::pair<std::vector<std::vector<float>>, std::vector<std::vector<float>>> PostProcessor::GetParamsKps(int stride_idx,
                                                                                                            float *x,
                                                                                                            std::vector<std::vector<float>> &coords,
                                                                                                            const std::vector<int> &xShape)
    {
        ...
        for (l = 0; l < coords.size(); l++)
        {
                ...
                for (ks_i = 0; ks_i < kernel_size; ks_i++)
                {
                    for (k_out = 0; k_out < output_channel1; k_out++)
                    {
                        ...
                        for (j = ks_j; j < ks_j + kernel_size; j++)
                        {
                            ...
                            {
                                for (k_in = 0; k_in < input_channel1; k_in = (k_in + 4))
                                {
            ...
            for (k_out = 0; k_out < output_channel2; k_out++)
            {
                ...
                for (j = 0; j < kernel_size; j++)
                {
                    for (i = 0; i < kernel_size; i++)
                    {
                        for (k_in = 0; k_in < input_channel2; k_in = (k_in + 4))
                        {
                            // For stride 4
                            feat_pos = k_in;
        ...
        // exit(1);
        return std::make_pair(outputParams, outputKps);
    }
