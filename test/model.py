...
def bn_fusion(value_lists):
    ...
    if M == 1:
        fused_weights = (kernel_weights * bn_gamma[None, None, :, None]
                         ) / np.sqrt(bn_moving_variance[None, None, :, None] +
                                     0.001)

    else:
        fused_weights = (kernel_weights *
                         bn_gamma) / np.sqrt(bn_moving_variance + 0.001)
...
class TestModel(tf.keras.Model):
    ...
    def __init__(self):
        ...
        self.dw1 = tf.keras.layers.DepthwiseConv2D(kernel_size=3,
                                                   strides=1,
                                                   use_bias=True,
                                                   padding='same',
                                                   name='dw_blk0')
        ...
        # self.dw2 = tf.keras.layers.DepthwiseConv2D(kernel_size=3,
        #                                            strides=2,
        #                                            use_bias=True,
        #                                            padding='same',
        #                                            name='dw0_blk0')  # 4

        # self.dw3 = tf.keras.layers.DepthwiseConv2D(kernel_size=3,
        #                                            strides=1,
        #                                            use_bias=True,
        #                                            padding='same',
        #                                            name='dw0_blk1')  # 5

        # self.dw4 = tf.keras.layers.DepthwiseConv2D(kernel_size=1,
        #                                            strides=1,
        #                                            use_bias=True,
        #                                            padding='same',
        #                                            name='dw1_blk1')  # 6

        # self.dw5 = tf.keras.layers.DepthwiseConv2D(kernel_size=1,
        #                                            strides=1,
        #                                            use_bias=True,
        #                                            padding='same',
        #                                            name='dw2_blk1')  # 7

        # self.dw6 = tf.keras.layers.DepthwiseConv2D(kernel_size=3,
        #                                            strides=1,
        #                                            use_bias=True,
        #                                            padding='same',
        #                                            name='dw3_blk1')  # 8
...
cp_dir = "/aidata/anders/data_collection/okay/total/archives/whole/scale_down"
path = "/aidata/anders/data_collection/okay/demo_test/imgs/frame-000867.jpg"
img = cv2.imread(path)
img = cv2.resize(img, (320, 320), 0.5, 0.5, interpolation=cv2.INTER_AREA)
img = img[..., ::-1]
img = img / 255.
...
preds = restored_model(img, training=False)
for multi_lv_feats in preds['multi_lv_feats']:
    cls_x, reg_dist, x = multi_lv_feats
    # multi_lv_feats = multi_lv_feats.numpy()
    cls_x = tf.reshape(cls_x, [-1])
    reg_dist = tf.reshape(reg_dist, [-1])
    
    x = tf.reshape(x, [-1])
    index = np.where(cls_x > 0.5)
