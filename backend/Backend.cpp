...
namespace SNN
{
    ...
    {
        this->bcfg = cfg;
        if (this->bcfg.precisionType == BackendConfig::Precision_FP32)
            this->permitFloat16 = true;

        if ((this->bcfg.backendType == BackendConfig::OpenCLImage) || (this->bcfg.backendType == BackendConfig::OpenCLBuffer))
        {
            this->_mBackend = (OpenCLBackend *)new OpenCLBackend(this->bcfg.backendType);
        }
        else
        {
    ...
    void Backend::BuildOperation(std::shared_ptr<Tensor> tensor, std::vector<std::shared_ptr<Execution>> &netOpContainer)
    {
        ...
        if (this->bcfg.backendType == BackendConfig::OpenCLImage)
        {
        ...
        else if (this->bcfg.backendType == BackendConfig::OpenCLBuffer)
        {
            if (tensor->GetOpType() == DEPTHWISECONV2D)
            {
                std::shared_ptr<DepthwiseConvExecution> op(new DepthwiseConvExecution(tensor, (OpenCLBackend *)this->_mBackend));
            ...
            {
                std::shared_ptr<ConvBufExecution> op(new ConvBufExecution(tensor, (OpenCLBackend *)this->_mBackend));
                bool status = op->onResize(tensor);
                netOpContainer.emplace_back(op);
                exit(1);
            }
    ...
    void Backend::PostOperators(std::vector<std::shared_ptr<Tensor>> &tensors, std::vector<std::shared_ptr<Execution>> &netOpContainer)
    {

        std::shared_ptr<PostProcessExecution> op(new PostProcessExecution(tensors, (OpenCLBackend *)this->_mBackend));
