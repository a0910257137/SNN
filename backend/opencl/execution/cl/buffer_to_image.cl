...
__kernel void nc4hw4_buffer_to_image(GLOBAL_SIZE_2_DIMS
  ...
  int buffer_offset =
      (((batch_idx + channel_block_idx * batch_size) * output_shape.x +
        height_idx) *
           output_shape.y +
       width_idx) *
      4;
...
__kernel void conv2d1x1_opt_filter_buffer_to_image(
    GLOBAL_SIZE_2_DIMS __global const FLOAT *input_ptr,
    __private const int input_channel, __private const int2 kernel_shape,
    __private const int ic_h_w_size, __private const int height_width_size,
    __write_only image2d_t output) {
...
__kernel void conv2d_filter_buffer_to_image(
    GLOBAL_SIZE_2_DIMS
  ...
  const int buffer_offset = output_channel_4_idx * ic_h_w_size +
                            input_channel_4_idx * height_width_size +
                            buffer_height_idx * kernel_shape.y +
                            buffer_width_idx;
  FLOAT4 output_values = 0;
...
__kernel void conv2d_filter_image_to_buffer(
    GLOBAL_SIZE_2_DIMS __global FLOAT *output_ptr,
    __private const int output_channel, __private const int2 kernel_shape,
    __private const int ic_h_w_size, __private const int height_width_size,
    __read_only image2d_t input_ptr) {
  int image_width_idx = get_global_id(0);
...
__kernel void nhwc_buffer_to_image(GLOBAL_SIZE_2_DIMS
  ...
  const int buffer_offset =
      ((batch_idx * height + height_idx) * width + width_idx) * channels + channel_4_idx;
...
__kernel void
arg_buffer_to_image(GLOBAL_SIZE_2_DIMS __global const FLOAT *input_ptr,
                    __private const int count, __write_only image2d_t output) {
  int image_width_idx = get_global_id(0);
  int image_height_idx = get_global_id(1);
...
// only for debug
__kernel void arg_image_to_buffer(GLOBAL_SIZE_2_DIMS __global FLOAT *output,
