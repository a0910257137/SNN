...
__kernel void
interp(GLOBAL_SIZE_3_DIMS __read_only image2d_t input,
  ...
  DEAL_NON_UNIFORM_DIM3(output_channel_block_idx, output_width_block_idx,
                        output_batch_height_block_idx);
  ...
  float4 out = read_imagef(
      input, SAMPLER,
      (int2)(input_width_offset + width_lf, input_height_offset + height_lf));

  const int out_image_w =
      mad24(output_channel_block_idx, output_width, output_width_block_idx);
  const int out_image_h =
      mad24(output_batch_idx, out_height, output_height_idx);

  write_imagef(output, (int2)(out_image_w, out_image_h), out);
}

__kernel void
interp3D(GLOBAL_SIZE_3_DIMS __read_only image2d_t input,
         __write_only image2d_t output, __private const float depth_scale,
         ...
         __private const float width_offset, __private const int input_depth,
...
//   printf("%d\n", output_channel_block_idx);
  ...
  const int input_image_width_offset =
      mul24(output_channel_block_idx, input_tensor_width_height);
  const int input_image_height_offset = mul24(output_batch_idx, input_depth);

  float4 out =
      read_imagef(input, SAMPLER,
                  (int2)(input_image_width_offset +
                             input_width * (height_offset + height_lf) +
                             width_lf + width_offset,
                         input_image_height_offset + depth_lf + depth_offset));

  const int output_image_width_offset =
      output_channel_block_idx * output_tensor_height_width;
  const int output_image_height_offset = output_batch_idx * out_depth;

  // TODO: out
  const int out_image_w = output_image_width_offset +
                          output_height_idx * out_width + output_width_idx;
  const int out_image_h = output_image_height_offset +
                          output_batch_idx * out_depth + output_depth_idx;
  write_imagef(output, (int2)(out_image_w, out_image_h), out);
}
