...
deconv_2d(GLOBAL_SIZE_3_DIMS
...
#endif
          __private const int2 input_shape, __private const int2 output_shape,
          __private const int2 stride_shape, __private const int2 align_shape,
          __private const int2 padding_shape, __private const int2 kernel_shape,
  ...
  DEAL_NON_UNIFORM_DIM3(out_channel_blocks_idx, out_w_idx,
                        out_batch_height_idx);
  ...
  int deal_kernel_width =
      kernel_shape.y - mad24(kernel_start_x, stride_shape.y, padding_shape.y) +
      out_w_idx - 1;
  int deal_kernel_height =
      kernel_shape.x - mad24(kernel_start_y, stride_shape.x, padding_shape.x) +
      out_h_idx - 1;
  ...
  for (int ic = 0; ic < in_channel_blocks; ic++) {
    ...
    for (int k_y = deal_kernel_height, idx_h = kernel_start_y; k_y >= 0;
      ...
      for (int k_x = deal_kernel_width; k_x >= 0; k_x -= stride_shape.y) {
        ...
        weights0 = vload4(kernel_x_0 * (out_channel_blocks * kernel_shape.x *
                                        kernel_shape.y) +
                              kernel_y,
                          weights);
        weights1 = vload4(kernel_x_1 * (out_channel_blocks * kernel_shape.x *
                                        kernel_shape.y) +
                              kernel_y,
                          weights);
        weights2 = vload4(kernel_x_2 * (out_channel_blocks * kernel_shape.x *
                                        kernel_shape.y) +
                              kernel_y,
                          weights);
        weights3 = vload4(kernel_x_3 * (out_channel_blocks * kernel_shape.x *
                                        kernel_shape.y) +
                              kernel_y,
                          weights);

        bool outBoundry = (idx_h < 0 || idx_h >= input_shape.x ||
                           kernel_start_x < 0 || in_width0 >= input_shape.y);
        int inp_offset =
            (((out_b_idx * in_channel_blocks + ic) * input_shape.x + idx_h) *
                 input_shape.y +
             in_width0) *
            4;
      ...
      for (int k_x = deal_kernel_width; k_x >= 0; k_x -= stride_shape.y) {
        ...
        in_width_value0 =
            select(in_idx + in_width_value0, -1,
                   (in_width_value0 < 0 || in_width_value0 >= input_shape.y));
  ...
  const int out_offset =
      (((out_b_idx * out_channel_blocks + out_channel_blocks_idx) *
            output_shape.x +
        out_h_idx) *
           output_shape.y +
       out_w_idx) *
...
#else
  int out_image_width_idx =
      mad24(out_channel_blocks_idx, output_shape.y, out_w_idx);
  WI_F(output, (int2)(out_image_width_idx, out_batch_height_idx), out0);
#endif
}

__kernel void iohw2oihw(__global const float *input_ptr,
                        __global float *output_ptr, int plane_number,
  ...
  const int input_offset =
      (ic_index * output_channel + oc_index) * plane_number;
  const int output_offset =
      (oc_index * input_channel + ic_index) * plane_number;
