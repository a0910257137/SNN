#define READ_INPUT_IMAGE(i, base)                                              \
  ...
  inOffset##i = select(inCurIdx + inOffset##i, -1,                             \
                       (inOffset##i < 0 || inOffset##i >= inputShape.y));      \
...
#endif
    void
    depthwise_conv2d_s1(GLOBAL_SIZE_2_DIMS __read_only image2d_t input,
  ...
  for (int kh = 0; kh < filterShape.x; kh++) {
    ...
    for (int kw = 0; kw < filterShape.y; kw++) {
      ...
      inWidthIdx = select(inCurIdx + inWidthIdx, -1,
                          (inWidthIdx < 0 || inWidthIdx >= inputShape.y));
...
__kernel
    ...
    depthwise_conv2d(
        ...
        __read_only image2d_t bias,
#endif
        __write_only image2d_t output, __private const int2 inputShape,
