...
__kernel void
conv_2d_1x1_local(GLOBAL_SIZE_3_DIMS __read_only image2d_t input,
                  __read_only image2d_t weights, __read_only image2d_t bias,
                  __write_only image2d_t output, __private const int in_c_block,
  ...
  for (int t = 0; t < tiles; ++t) {
    ...
    in_sm1[in_index] =
        RI_F(input, SAMPLER, (int2)(in_c_w_idx + 1, out_b_h_idx));
    in_sm2[in_index] =
        RI_F(input, SAMPLER, (int2)(in_c_w_idx + 2, out_b_h_idx));
    in_sm3[in_index] =
        RI_F(input, SAMPLER, (int2)(in_c_w_idx + 3, out_b_h_idx));
...
__kernel
...
#endif
    void
    conv_2d_1x1(GLOBAL_SIZE_2_DIMS __read_only image2d_t input,
                __read_only image2d_t weights, __read_only image2d_t bias,
  ...
  intput_width_idx1 =
      select(intput_width_idx1, INT_MIN, intput_width_idx1 >= input_shape.y);
  intput_width_idx2 =
      select(intput_width_idx2, INT_MIN, intput_width_idx2 >= input_shape.y);
  intput_width_idx3 =
      select(intput_width_idx3, INT_MIN, intput_width_idx3 >= input_shape.y);
  ...
  FLOAT4 weights3;
  // in##i
  for (int in_channel_block_idx = 0; in_channel_block_idx < in_channel_block;
    ...
    in0 = RI_F(
        input, SAMPLER,
        (int2)(input_width_base + intput_width_idx0, input_height_block_idx));
    in1 = RI_F(
        input, SAMPLER,
        (int2)(input_width_base + intput_width_idx1, input_height_block_idx));
    in2 = RI_F(
        input, SAMPLER,
        (int2)(input_width_base + intput_width_idx2, input_height_block_idx));
    in3 = RI_F(
        input, SAMPLER,
        (int2)(input_width_base + intput_width_idx3, input_height_block_idx));
    weights0 = RI_F(weights, SAMPLER,
                    (int2)(weights_width_base + 0, output_channel_block_idx));
    weights1 = RI_F(weights, SAMPLER,
                    (int2)(weights_width_base + 1, output_channel_block_idx));
    weights2 = RI_F(weights, SAMPLER,
                    (int2)(weights_width_base + 2, output_channel_block_idx));
    weights3 = RI_F(weights, SAMPLER,
                    (int2)(weights_width_base + 3, output_channel_block_idx));
    CALCULATE_OUTPUT(0);
    CALCULATE_OUTPUT(1);
    CALCULATE_OUTPUT(2);
    CALCULATE_OUTPUT(3);
...
__kernel
...
#endif
    void
    conv_2d_c4h1w4(GLOBAL_SIZE_2_DIMS __read_only image2d_t input,
  ...
  int in_width0 =
      mad24(out_height_block_idx, stride_shape.y << 2, -padding_shape.y);
  ...
  const int height_start =
      mad24((output_batch_height_idx % output_shape.x), 1, -padding_shape.x);
  int in_height_start =
      select(0, (-height_start), height_start < 0) + height_start;
  ...
  const int weights_h_idx =
      ...
      mul24(select(0, (-height_start), height_start < 0), weights_shape.y);
#else
...
__kernel
...
#endif
    void
    conv_2d_c4h4w1(GLOBAL_SIZE_2_DIMS __read_only image2d_t input,
                   __read_only image2d_t weights,
#ifdef BIAS
                   __read_only image2d_t bias,
#endif
  ...
  for (int in_channel_block_idx = 0;
    ...
    int weights_x_idx = in_channel_block_idx << 2;
    int weights_y_idx = weights_h_idx;
    for (int iy = 0; iy < weights_shape.x * dilation_shape.x;
         iy += dilation_shape.x) {
      int h0 =
          select(in_height0 + iy + batch_idx, -1,
                 (in_height0 + iy < 0 || in_height0 + iy >= input_shape.x));
      int h1 =
          select(in_height1 + iy + batch_idx, -1,
                 (in_height1 + iy < 0 || in_height1 + iy >= input_shape.x));
      int h2 =
          select(in_height2 + iy + batch_idx, -1,
                 (in_height2 + iy < 0 || in_height2 + iy >= input_shape.x));
      int h3 =
          select(in_height3 + iy + batch_idx, -1,
                 (in_height3 + iy < 0 || in_height3 + iy >= input_shape.x));
      for (int ix = 0; ix < weights_shape.y * dilation_shape.y;
           ix += dilation_shape.y) {
        int w0 =
...
__kernel
...
#endif
    void
    conv_2d_c8h4w1(GLOBAL_SIZE_2_DIMS __read_only image2d_t input,
                   __read_only image2d_t weights,
#ifdef BIAS
                   __read_only image2d_t bias,
#endif
  ...
  FLOAT4 in0, in1, in2, in3;
  FLOAT4 weights0, weights1, weights2, weights3, weights4, weights5, weights6,
      weights7;
  for (int in_channel_block_idx = 0;
       in_channel_block_idx < in_channel_block_length; ++in_channel_block_idx) {
    const int in_idx = mul24(in_channel_block_idx, input_shape.y);
    ...
    for (int iy = 0; iy < weights_shape.x * dilation_shape.x;
      ...
      for (int ix = 0; ix < weights_shape.y * dilation_shape.y;
        ...
        weights4 = RI_F(weights, SAMPLER,
                        (int2)(weights_x_idx + 0, weight_size + weights_y_idx));
        weights5 = RI_F(weights, SAMPLER,
                        (int2)(weights_x_idx + 1, weight_size + weights_y_idx));
        weights6 = RI_F(weights, SAMPLER,
                        (int2)(weights_x_idx + 2, weight_size + weights_y_idx));
        weights7 =
